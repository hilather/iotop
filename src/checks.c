...
inline int system_checks(void) {
	...
	/* Check self, not PID 1 — PID 1 may be invisible in some namespaces. */
	if (!is_a_process(getpid())) {
		fprintf(stderr,"Looks like /proc is not mounted or not usable.\n");
		return 1;
	}
	...
	if (!io_accounting||!vm_event_counters) {
		fprintf(stderr,"Could not run iotop as some of the requirements are not met:\n");
		fprintf(stderr,"- Linux >= 2.6.20 with\n");
		if (!io_accounting)
			fprintf(stderr,"  - I/O accounting support (CONFIG_TASKSTATS, CONFIG_TASK_DELAY_ACCT, CONFIG_TASK_IO_ACCOUNTING)\n");
		if (!vm_event_counters)
			fprintf(stderr,"  - VM event counters (CONFIG_VM_EVENT_COUNTERS)\n");
		return 1;
	}
	return 0;
}
